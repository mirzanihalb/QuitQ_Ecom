    ...
    public class OrderRepositoryImpl:IOrder
    {
        ...
        public async Task<List<OrderDTO>> ViewAllOrdersByUserId(int userId)
        {
            var orders = await _context.Orders
                .Include(o => o.OrderItems) // Eager loading OrderItems
                    .ThenInclude(oi => oi.Product) // Eager loading Product information for each OrderItem
                .Include(o => o.Shipper) // Eager loading Shipper information
            ...
            var orderDTOs = orders.Select(o => new OrderDTO
            {
                ...
                Shipper = o.Shipper != null ? o.Shipper.ShipperName : null, // Include ShipperName if Shipper is not null
                orderItemListDTOs = o.OrderItems.Select(oi => new OrderItemDTO
                {
                    ...
                    // Include ProductDTO information if Product is not null
                    Product = oi.Product != null ? new ProductDTO
                    {
                        // Map properties from Product entity to ProductDTO
                        ProductId = oi.Product.ProductId,
                        ProductName = oi.Product.ProductName,
                        ProductImage = oi.Product.ProductImage,
                        // Add other properties as needed
                    } : new ProductDTO // Create a placeholder ProductDTO for null Product
                    {
                        // Set properties to indicate null Product
                        ProductId = -1, // Or any other appropriate value
                        ProductName = "Product Not Available", // Or any other appropriate message
                                                               // Add other properties as needed
                    }
                }).ToList()
            }).ToList();
        ...
        public async Task<List<OrderDTO>> ViewOrdersByStoreId(int storeId)
        {
            try
            {
                ...
                //    .Where(o => o.OrderItems.Any(oi => oi.Product.StoreId == storeId)) // Filter orders by seller's store ID
                //    .ToListAsync();

                var orders = await _context.Orders
    .Include(o => o.OrderItems)
        .ThenInclude(oi => oi.Product)
    .Where(o => o.OrderItems != null && o.OrderItems.Any(oi => oi.Product != null && oi.Product.StoreId == storeId)) // Filter orders by seller's store ID
    .ToListAsync();
        ...
        .Where(oi => oi.Product != null && oi.Product.StoreId == storeId) // Add null check for oi.Product
