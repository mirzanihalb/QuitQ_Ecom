...
namespace QuitQ_Ecom.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        [HttpPut("{userId:int}")]
        public async Task<IActionResult> UpdateUserDetails(int userId, [FromBody] UserDTO updatedUserDto)
        {
            try
            {
                var existingUser = await _userRepo.GetUserByIdAsync(userId);
                string hashedPassword = HashPassword(updatedUserDto.Password);
                updatedUserDto.UserId = userId;
                ...
                var updatedUser = await _userRepo.UpdateUserDetails(updatedUserDto);
            ...
            {
                _logger.LogError(ex, $"An error occurred while updating user details: {ex.Message}");
                return StatusCode(500, $"Internal server error: {ex}");
            }
        }
        ...
        public async Task<IActionResult> ResetPassword([FromBody] ResetPasswordDTO resetPasswordDto)
        {
            ...
            try
            {
                bool resetSuccessful = await _userRepo.ResetPassword(resetPasswordDto.email, resetPasswordDto.newPassword);
                if (!resetSuccessful)
                {
            ...
            {
                _logger.LogError(ex, $"An error occurred while resetting password: {ex.Message}");
                return StatusCode(500, $"Internal server error: {ex}");
            }
        }
    }
