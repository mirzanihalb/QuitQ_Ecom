...
namespace QuitQ_Ecom_Test
{
    ...
    public class OrderControllerTest
    {
        ...
        public async Task Orders_ExistingSellerId_ReturnsOk()
        {
            ...
            _orderRepoMock.Setup(repo => repo.ViewOrdersByStoreId(storeid)).ReturnsAsync(orders);

            // Act
            var result = await _orderController.Orders(storeid);
        ...
        public async Task Orders_NonExistingSellerId_ReturnsOk()
        {
            // Arrange
            int storeid = 1;
            _orderRepoMock.Setup(repo => repo.ViewOrdersByStoreId(storeid)).ReturnsAsync(new List<OrderDTO>());
